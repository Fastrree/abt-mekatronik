    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-50 bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl overflow-hidden w-[110px] sm:w-[160px]">
            {languages.map((lang) => (
              <button
                key={lang.code}
                onClick={() => handleSelect(lang.code)}
                className={`w-full flex items-center gap-1.5 sm:gap-2 px-2 py-1.5 sm:px-2.5 sm:py-2 text-left hover:bg-zinc-700 transition-colors ${
              ...
              >
                ...
                <span className="text-[9px] sm:text-sm font-medium truncate">{lang.name}</span>
              </button>
            ))}
          </div>
        </>
